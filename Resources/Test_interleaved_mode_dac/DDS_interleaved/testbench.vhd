...
BEGIN
	...
	BEGIN
		...
		-- Case 2 - Increment = 1 - Phase shift = 1
		s_i_n_reset	<= '1';
		s_i_increment <= STD_LOGIC_VECTOR(TO_UNSIGNED(1, PHASE_WIDTH));
		s_i_phase_shift	<= STD_LOGIC_VECTOR(TO_UNSIGNED(1, PHASE_WIDTH));
		WAIT FOR ((2**(PHASE_WIDTH+1) / 2 + 10) * PERIOD);
		...
		-- Case 3 - Increment = 2 - Phase shift = 0
		s_i_n_reset	<= '1';
		...
		-- Case 4 - Increment = 2 - Phase shift = 1
		s_i_n_reset	<= '1';
		s_i_increment <= STD_LOGIC_VECTOR(TO_UNSIGNED(2, PHASE_WIDTH));
		s_i_phase_shift	<= STD_LOGIC_VECTOR(TO_UNSIGNED(1, PHASE_WIDTH));
		WAIT FOR ((2**(PHASE_WIDTH+1) / 2 + 10) * PERIOD);
